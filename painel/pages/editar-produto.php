...
<h2><i class="fa fa-pencil"></i> Editando Produto: <?php echo $id; ?></h2>
	...
	<form method="post" action="<?php echo INCLUDE_PATH_PAINEL ?>editar-produto?id=<?php echo $id; ?>" enctype="multipart/form-data">
		<?php
			if(isset($_POST['acao'])){
				...
				if($sucesso){
					...
					$sql = MySql::conectar()->prepare("UPDATE `tb_admin.estoque` SET nomw = ?,descricao = ?,
						altura = ?, lergura = ?, comprimento = ?, peso = ?, quantidade = ? WHERE id = $id");
		...
		<div class="form-group">
			...
			<input type="number" name="largura" min="0" max="900" step="1" value="<?php echo $infoProduto['lagura'] ?>">
		</div><!--form-group-->
