<?php
	...
	if(isset($_GET['deletarImagem'])){
		...
		$pegaImagens->execute();
		$pegaImagens = $pegaImagens->fetchAll();
	}else if(isset($_GET['deletaarImovel'])){
		...
		Painel::alert("O imóvel foi deletado com sucesso!");
...
<h2><i class="fa fa-pencil"></i> Editando Imóvel: <?php echo $id; ?></h2>
	...
	<div class="form-group">
		...
		<a class="btn delete" href="<?php echo INCLUDE_PATH_PAINEL ?>editar-imovel?id=<?php echo $id; ?>&deletarImovel"><i class="fa fa-times"></i> Excluir Imóvel</a>


	</form>
<div class="card-title">Imagens do imóvel:</div>
	<div class="boxes">
		<?php
			foreach($pegaImagens as $key => $value){
		?>
